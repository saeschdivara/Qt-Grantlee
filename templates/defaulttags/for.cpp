...
//   }
  ...
  if (!QMetaType::hasRegisteredConverterFunction(varFE.userType(), qMetaTypeId<QtMetaTypePrivate::QSequentialIterableImpl>()))
  {
    if (varFE.userType() != qMetaTypeId<QVariantList>() && varFE.userType() != qMetaTypeId<QStringList>() ) {
      c->pop();
      return m_emptyNodeList.render( stream, c );
    }
  }
